...
class MinerServerApi extends server_api_1.ServerApi {
    ...
    init(expressApp) {
        ...
        expressApp.post("/miner/update", (req, res) => {
            ...
            let item = new miner_session_model_2.MinerWorkItemUpdate(form.id, form.sessionId, form.accepted, form.title, form.price, form.message);
            ...
            this.minerDb.updateWorkQueue(item).then((result) => {
                ...
                let updateRes = new miner_session_model_1.WorkItemUpdateRes(form.id, form.sessionId, success);
                res.json(updateRes);
            }).catch((err) => {
