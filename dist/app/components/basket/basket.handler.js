...
class BasketHandler {
    ...
    getSessionBasket(session) {
        return session.get(session_keys_1.SessionKeys.Basket);
    }
    ...
    getVendorBasket(session, vendorId) {
        let result = null;
        let sessBasket = this.getSessionBasket(session);
     ...
     * @returns {boolean}
     */
    removeProductByCode(session, code, basket = null) {
        let result = false;
        if (basket === null) {
            basket = this.getSessionBasket(session);
        }
        ...
        session.setValue(session_keys_1.SessionKeys.Basket, basket);
        return result;
    }
    /**
