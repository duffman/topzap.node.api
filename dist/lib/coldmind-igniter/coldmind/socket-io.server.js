...
const zyn_sio_session_1 = require("@igniter/coldmind/zyn-sio-session");
class SocketEntry {
...
class SocketServer {
    ...
    emitToSession(sessId, messageType, mess) {
        let entry = this.getSessionSocketEntry(sessId);
        console.log("sendToSessionId :: sessId ::", sessId);
        if (entry === null) {
    ...
    handleMessage(message, socket) {
        ...
        let socketSession = new zyn_sio_session_1.ZynSioSession(socket);
        ...
        this.eventEmitter.emit(igniter_event_types_1.SocketEvents.NewMessage, igniterMessage, socketSession);
    }
